...
Relational algebra was created during the 1960s and 70s by Edgar F. Codd, who worked at IBM at the time. Codd proposed this formal language as a basis for database querying.

The good news is that knowing set theory and relational algebra reduces potential SQL-related problems to basically documentation lookup. If you understand how unions work, the idea behind Cartesian products, and what expressions of relational algebra are, then it comes down to finding the correct SQL syntax to solve the problem at hand.
...
A **relation** is essentially a table, with columns (called "attributes") and rows. For example, a relation =User= may be used to describe users in a web service.
...
A **projection** is an operation of extracting records with specific columns (attributes). It is denoted by the Greek letter $\Pi$ (uppercase pi), with a list of columns as subscript, followed by a relation's name.
...
A **selection** is an operation of filtering records by values. It is denoted by the Greek letter $\sigma$ (sigma) with a condition, followed by the relation's name.

For example, here we get managers with a level higher than 2:
...
Select and project operators work on any expression, not just whole relations. This means we can combine them arbitrarily. For example, we can select (essentially filter) first, and then extract certain columns.
...
Recall the idea behind the Cartesian product in set theory: given two sets $A = \{a, b\}$ and $B = \{1, 2\}$, the *Cartesian product* is a set of pairs of all combinations of elements:
...
Much of the power of relational algebra comes from applying this idea to relations. It is also often called a *cross-product*.
...
This might not seem too useful: we just combined all records, and many of the new rows don't make sense. Cross-products are rarely useful on their own. Instead, the goal is often to generate raw data for operations. For example, with this long table at hand, we can first eliminate the nonsense rows by applying select:
...
As you can see, a natural is basically syntactic sugar on top of the existing features of relational algebra.
...
All cases can be thought of as two sets, with a rule to connect their elements.
